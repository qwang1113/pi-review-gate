...
- **复用是有界的，边界由门禁划、你不需要知道**（2026-09-05 用户口径，
  \`lib/judge-rotation.ts\`）：复用单元是**一个已批准的 review 对象**（loop 会话的
  goal / 编排会话的 plan，都还没批准时是一个稳定占位对象）；对象一换、或你自报的
  上下文占用过 60%、或同一对象已派满 8 轮，门禁就给你开一条**新 transcript**，
  只带压缩交接（上轮裁决 + 未关闭 findings + 自上次审查以来的 delta），不带历史。
  轮转是门禁的决定：没有开关、没有配置、你也不用申请。你只会看到任务文本开头多一句
  「这是一条新 transcript」——它的意思就是**前几轮的对话不在你的上下文里，任务文本
  就是全部依据**，别去回忆、也别假设自己上轮说过什么。旧 transcript 原地保留（目录不删，
  按既有 TTL 回收）。
