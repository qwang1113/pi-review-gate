...
export function buildPlanAuditTask(
  ...
  const instructions = [
    ...
    "   （repo 决定子会话的 cwd与它拿到哪个 checkout；写错仓库是 2026-09-01 实测过的死锁。）",
    ...
    "3. 并行是否真的可并行：同一 repo 里多个任务现在**真的能并行** —— 第二个写者",
    "   自动拿到自己的 `git worktree`（独立分支，共享对象库）。所以不要因为\"同一个 repo\"就",
    "   判它不可并行，那是 2026-09-10 之前的规则。当问的是另一件事：**它们会不会改到同一批文件**？",
    "   两个任务动同一批文件会在结算时冲突（门禁会中止合并、不丢工作，但那一轮白跑了）——",
    "   这种才该改成串行或拆开写面。",
    ...
    "8. 需求是否已澄清、goal 是否可派生（PM=产品经理，2026-09-17）：",
    ...
    "10. 最后两环的分工：倒数第二个是收尾任务，最后一个必须是独立验收任务（2026-09-22，用户决定）：",
    "   plan 的**最后两个任务**构成交付的两环，位置约定（倒数第二 = 收尾任务，最后 = 验收任务），",
    "   不是 plan 的新字段。它们各自的站点由 lib/repo-pr-policy.ts 的 `acceptanceTaskId` 与",
    "   `effectiveTaskStation` 唯一决定 —— 去读那两个函数，不要在任务书里另立一套。",
    "   - **倒数第二个 = 收尾任务**：汇合其余任务的成果 → 走一次整体审核 → commit。它照旧计入该 repo",
    "     的任务数，因此按同一 repo 规则收窄（提交完就停）。",
    "   - **最后一个 = 独立验收任务**：任务书写明「不产出新需求、不改业务代码、只做真实验收与交付」——",
    "     真实验收是跑真实路径 / 命令 / 观察（不是复述实现，也不是给自己打分）；交付物写清（已 push 的分支 / PR 链接）。",
    "     只有它不受同一 repo 多任务的收窄影响，它的站点就是 plan.deliveryStation。",
    "   以下任一情形是 P1：",
    "   (a) 最后一个任务仍是「实现某个功能」的任务，或它的任务书看不出「不产出新需求、不改业务代码、只做真实验收」",
    "       —— 验收由实现它的那个子会话自己说了算，等于自评。",
    "   (b) plan 里有多个产出任务（要汇合），而倒数第二个仍是实现任务 —— 汇合与整体审核没人做，",
    "       最后一个验收任务得先合并甚至解冲突：验收被掺杂，而它本来就被要求不改业务代码。",
    "   (c) 整个 plan 没有一个能把成果交付出去的任务（谁 push？谁开 PR？）—— 项目经理被硬约束禁止 ship",
    "       （不写代码，只放行 plan 与交接文档），同 repo 多任务的子会话又被收窄到 `commit`，",
    "       于是**没有任何一方能开 PR**，整轮卡在交付上（2026-09-18 实测的事故）。",
    "   用仓库事实判：读最后两个任务的 note，各自的交付物与分工写没写清楚；顺序对不对（收尾在前、验收在后，",
    "   `dependsOn` 或 plan 顺序上都在其余任务之后），否则验收的是半成品。",
    "",
    "",
