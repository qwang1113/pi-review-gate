...
3. **寻址用 orchestration id**（`RG_ORCHESTRATION_ID`），不是 session id：接力
   ...
   本身是**退休四件事**（停两个推进定时器、释放 worktree 占用、三条唤醒路径全部静默、
   停止写共享 sidecar），发生在开新 pane **之前**、交不成可回滚；继任者带着**前任的
   session id** 接管 worktree 占用，所以前任没来得及释放也接得上。「退休四件事」与
   「继任凭据」的权威出处只有一处：`docs/execution-model.md` 的「接力的不断档保证」。
