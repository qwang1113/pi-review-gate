...
// ---------------------------------------------------------------------------
// PI-SELF (USER REQUIREMENT): the gate never gates its own repository — a
// repo whose package.json name is "pi-review-gate" is exempt from the verdict
// checks and the commit-msg check, deterministically (the repo identity is
// chosen by the user, not forgeable from a hook).
// ---------------------------------------------------------------------------

test("PI-SELF: verdict checks are exempt in the gate's own repo (pre-commit)", () => {
  const dir = makeDir();
  writeState(dir, {
    ...READY,
    review: { verdict: "PENDING", fingerprint: null, at: null },
    precommit: { verdict: "NOT_RUN", fingerprint: null, at: null },
    taskModeSource: "auto", // would NOT exempt an ordinary repo
  });
  writeFileSync(join(dir, "package.json"), JSON.stringify({ name: "pi-review-gate" }));
  assert.equal(runPreCommit(dir).status, 0);
});

test("PI-SELF: exempts even a BLOCKED sidecar in the gate's own repo", () => {
  const dir = makeDir();
  writeState(dir, {
    ...READY,
    review: { verdict: "BLOCKED", fingerprint: null, at: null },
    precommit: { verdict: "FAIL", fingerprint: null, at: null },
  });
  writeFileSync(join(dir, "package.json"), JSON.stringify({ name: "pi-review-gate" }));
  assert.equal(runPreCommit(dir).status, 0);
});

test("SECURITY: a same-shaped package.json with a different name stays enforced", () => {
  const dir = makeGitRepo();
  writeState(dir, {
    ...READY,
    review: { verdict: "PENDING", fingerprint: null, at: null },
...
test("PI-SELF: commit-msg skips the AI-attribution check in the gate's own repo", () => {
  const dir = makeGitRepo(); // the hook resolves the repo root via git
  writeFileSync(join(dir, "package.json"), JSON.stringify({ name: "pi-review-gate" }));
  const msgFile = join(dir, "COMMIT_EDITMSG");
  writeFileSync(msgFile, "feat: x\n\nGenerated by AI assistant");
  const res = spawnSync("bash", [COMMIT_MSG, msgFile], { cwd: dir, encoding: "utf8", env: { ...process.env } });
...
function runCommitMsg(message: string, env: Record<string, string> = {}) {
  ...
  // cwd must be the repo dir — git invokes hooks from the repo root and the
  // hook's pi-self check resolves the repo via git, so running it from the
  // test process's own cwd would judge the WRONG repository.
  return spawnSync("bash", [COMMIT_MSG, msgFile], { cwd: dir, encoding: "utf8", env: { ...process.env, ...env } });
}
