...
test("P0 regression: clustered boolean shorthands before -m are extracted (-am/-sam)", () => {
  assert.deepEqual(extractCommitMessages("git commit -am 'Generated by Claude'"), ["Generated by Claude"]);
  assert.deepEqual(extractCommitMessages('git commit -sam "msg body"'), ["msg body"]);
  assert.deepEqual(extractCommitMessages("git commit -samfix"), ["fix"]);
});
test("clustered extraction does NOT misread other value-taking shorthands", () => {
  // -C takes a commit ref (reuse message), not an inline message.
  assert.deepEqual(extractCommitMessages("git commit -C abc123"), []);
  assert.deepEqual(extractCommitMessages("git commit --amend --no-edit"), []);
});
