...
| ~~`judge_read` / `judge_wait` / `judge_close`~~ | REMOVED — manual round management (polling, reading conclusions, closing panes) now belongs to the gate: a new channel report wakes the opener with the standard report (verdict, evidence pointer, record note, open questions). The implementations stay on the gate's internal host for its own audit chains. |
