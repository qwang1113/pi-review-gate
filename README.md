...
| `orchestrator_send` | Message a child — including approving its loop goal on the user's behalf, which is boundary-checked first. |
| `orchestrator_wait` | Block until something actually happens (see below). |
| `orchestrator_notify` | The **only** channel to the human who is not watching the terminal. |
| `orchestrator_relay` | Hand the whole orchestration to a successor session. |
| `orchestrator_close` | Close a pane this orchestration owns — nothing else is addressable. |
| `orchestrator_status` | Read the entire orchestration back in one call (how a successor picks up). |
...
**Waiting is a tool, not a loop.** `orchestrator_wait` reuses the generic skeleton (`lib/poll-wait.ts`) with its own criteria, and they are not a judge's: an orchestration child is interactive, so it does **not** exit when it finishes — waiting for a process to end would hang forever. The end states are events: an attention event from a child, a child reporting done, its pane vanishing, or the timeout.
...
| `orchestrator_plan` / `orchestrator_spawn` / `orchestrator_send` / `orchestrator_wait` / `orchestrator_notify` / `orchestrator_relay` / `orchestrator_close` / `orchestrator_status` | The orchestration layer, available only in `orchestrator` mode — see [The orchestrator role](#the-orchestrator-role-a-project-manager-inside-the-gate). The decisions live in `lib/orchestrator-*.ts` (plan state machine, file-boundary algebra, tmux argv construction, the bash backstop, the 14 constraints, the relay protocol); the extension only wires them up. |
