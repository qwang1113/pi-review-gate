...
**Only `/tmp` scratch sessions are exempt by rule.** A session **started in the scratch dir `/tmp`** (macOS `/private/tmp` is the same dir through a symlink) is classified `normal` automatically on the first `set_gate_mode` call (`lib/pi-self.ts`, deterministic path detection: the session's repo root is chosen by the user, so this is not a forgeable text claim). No LLM round-trip, no dialog: `/tmp` is scratch space by definition. **Nothing else is exempt** — editing `~/.pi` (settings, MCP, installed extensions), the pi binary's install dir, or developing this repository itself all run the full loop like any development.
