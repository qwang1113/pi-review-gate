...
**Pi's global config is exempt by rule.** A session working on **pi's global configuration** — the user's `~/.pi` config dir (settings, MCP, installed extensions) or the pi binary's install dir — or troubleshooting pi's own behavior, is classified `normal` automatically on the first `set_gate_mode` call (`lib/pi-self.ts`, deterministic path detection: the session's repo root is chosen by the user, so this is not a forgeable text claim). No LLM round-trip, no dialog: pi's own config is not gate material. Requesting `loop` still wins (the user can always demand the full loop). The LLM classifier prompt teaches the same rule as a soft fallback for pi-config tasks in unrelated directories. **Deliberately NOT exempt: developing pi-review-gate itself** — its repository runs the full loop like any development, and per-project `.pi` installs are project config, not global. The git hooks need no mirror (pi's config dirs are not git repos), so every repo — the gate's own checkout included — stays fully enforced.
