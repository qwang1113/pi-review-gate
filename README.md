...
|---|---|
| `orchestrator_plan` | Read/replace the plan, submit it for the **user's** approval, move a task through its state machine, record and resolve decisions only the human can settle. |
...
> **Ten entries left this table on 2026-08-30** (philosophy three: never run
> two implementations of the same thing). `run_precommit`, `review_checkpoint`,
> `prepare_review`, `prepare_adviser`, `prepare_goal_audit`, `record_review`
> and `record_goal_prereview` are **still the implementations** the gate runs
> — `judge_submit` and `propose_loop_goal` call them internally, so every
> mechanical check lives in exactly one place — but they are no longer
> registered with pi and a model cannot see the names. `review_spawn`,
> `review_watch` and `review_send` were deleted outright. The rows for the
> seven survivors are left below **unchanged**, because what they *do* is
> still exactly how the gate works and worth reading — they are simply not
> things you can call. Anything in them that reads like an instruction to the
> agent ("ADVANCED / internal — call this only when…") is now describing the
> gate's own internal step.
