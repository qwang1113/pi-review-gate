...
**Addressing is by orchestration, not by session.** A child's wake-ups are stamped with `RG_ORCHESTRATION_ID`, not with the id of the session that spawned it — so when a relay hands the role to a successor, every running child keeps reaching whoever holds it now, with nothing restarted and nothing re-stamped. The relay's closing move is the guarantee that there is no gap: **only the successor may close the predecessor**, which means the old session stays alive until something demonstrably running has taken over. It inherits the id, the handoff document, a pointer to the predecessor's **transcript** (a handoff document is a self-report; the raw record is what you need when the plan later goes wrong), and the predecessor's **session id** — its proof of heirship. Retiring the predecessor is four things, not a flag (2026-09-10, after a measured failure): stop both wake-up timers, **release the worktree claim** (the successor arms its gate in the SAME checkout — releasing after the pane opens is racing its boot, and it lost: the successor was refused by its own predecessor's heartbeat and its pi exited, leaving the pane on a bare shell), guard **every** wake-up path (`orchestratorSettled` had none, so `agent_settled` put the predecessor back into `orchestrator_wait` two seconds after a successful handoff — two project managers, one plan), and stop writing the shared sidecar. All of it happens BEFORE the successor's pane opens, and all of it is rolled back if that pane never starts; the successor's heirship claim means even an un-released claim cannot block the handoff.
