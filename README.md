...
WORKERS (2026-09-21). `worker_submit({task, workerId?, role?, model?})` dispatches a READ-ONLY pane session — the pane-shaped replacement for `npm:@tintinweb/pi-subagents` (`Agent` / `SubagentWorkflow`), and the answer to "opening a subagent and opening a review pane should be the same thing". Same pane factory, same channel, same deterministic session id; `--exclude-tools edit,write` is the read-only contract (the tool surface, not a rule), which is what makes several workers safe to run at once. A LIVING pane receives a second submit as a message (not a second worker); a CLOSED one is re-opened under the same session id, so `worker_close` frees screen space without losing the conversation. `worker_wait` returns the worker's report or the question it is blocked on, `worker_answer` answers it. What each preset runs is configured in `~/.pi/review-gate.json` under `agents.worker*` (a model chain plus its own `prompt`); an unconfigured preset FAILS the dispatch instead of running on a default model.
