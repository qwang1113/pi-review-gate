...
**Judge roles (reviewer / adviser / goal-auditor / quality-auditor) run in their OWN tmux panes** — interactive pi with a deterministic session id per role+repo, `--exclude-tools edit,write`, resumed by that id when the pane is gone. No workflow engine, no background subagent.
