...
**A session started OUTSIDE any git repository is not path-exempt — it has nothing to gate.** There is no repo to review, checkpoint or ship, so the extension short-circuits entirely (2026-09-02, user decision): it forces `normal`, calls no git at all (the old behavior called git anyway and leaked `fatal: not a git repository` to the terminal), shows `非 git 目录` in the status strip, and refuses `checkpoint` / `judge_submit` / `run_precommit` / `record_review` with the reason; `set_gate_mode loop|orchestrator` is clamped to `normal` exactly like the `/tmp` rule. The output-language directive (L4) stays — it is orthogonal to the gate.
...
| `judge_read` | Adviser-only snapshot — never a wait: its channel state, open questions, and transcript conclusion. Reviewer / goal-auditor go through `judge_wait` and the channel report instead. Only the opener may read. |
| `judge_wait` | Block until YOUR OWN judge's current round is over, then return what it produced. This is the FALLBACK, not the normal path — call it only when there is genuinely nothing else to do. A NEW channel report ends the wait (the gate records it), a dead pane ends it as failed (recover or close). On timeout it returns the current state instead of failing, so the decision stays with the agent. Only the opener may wait. |
| `judge_close` | Close YOUR OWN judge pane and drop it from the registry. Not a memory wipe: the transcript stays on disk, so the same id re-opens the same conversation. Idempotent. `declare_done` cascade-closes whatever panes are still open (recorded verdicts stay recorded, unrecorded rounds are abandoned). |
