...
**The plan is the exit contract**, and it is the orchestration-layer twin of the loop goal: writing `.pi/orchestrator-plan.json` grants nothing, because the approval binds to the **content hash** the user saw in a dialog the extension rendered. Widen a boundary, add a task, or move a task to another repo afterwards and the approval is gone. Every task must declare the files it may touch — that one field is what makes scheduling and proxy-approval decidable at all. A task may also declare a `repo` (2026-09-07): children of the SAME repo are never co-scheduled (one checkout, one writer), while tasks in DIFFERENT repos run in parallel — the old isolated-worktree machinery that made same-repo parallelism possible is gone. A goal the orchestrator approves for a child may not name a path outside its task (that is a scope change, and scope belongs to the human).
