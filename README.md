...
Each session runs in one of **four gate modes** (strictness order `normal < explore < loop < orchestrator`), decided via the `set_gate_mode` tool — and the agent's own pick **is** the classification: no external classifier model is consulted for the mode. What makes that safe is not a second opinion but the rule engine's asymmetry (`lib/task-mode.ts`) — the agent can classify itself **into** the gate but never **out of** it. A first `loop` (or `orchestrator`) always applies, a first `explore` applies while this session is still clean, and all of them record `source: "auto"` so the git hooks stay fully enforced; a first `normal` (the gate switching off entirely) always pops the user's confirmation dialog. That is the whole story everywhere; in a Temp dir (`/tmp`) the gate additionally nudges trivial work toward `normal` — never forced. While the mode is undecided the gate behaves exactly like `loop` (fail-closed) and the per-turn system prompt instructs the agent to call `set_gate_mode` as its first action; “the agent never decides” therefore costs nothing.

**Only non-git directories are path-exempt, and they never enter enforced modes via the agent.** A session **started outside any git repository** has nothing to review/checkpoint/ship, so the first classification applies `normal` automatically (consent-free, driven by the deterministic session cwd). A `loop`/`orchestrator` pick is clamped to `normal`; a later agent upgrade is rejected; only the user can force an enforced mode (`/gate-mode loop`). **Temp dirs (`/tmp`) are NOT exempt** — the gate only nudges trivial work toward `normal` there; delivery work runs the same modes as anywhere else.

**A session started OUTSIDE any git repository has nothing to gate.** There is no repo to review, checkpoint or ship, so the extension short-circuits entirely (2026-09-02, user decision): it forces `normal`, calls no git at all (the old behavior called git anyway and leaked `fatal: not a git repository` to the terminal), shows `非 git 目录` in the status strip, and refuses `checkpoint` / `judge_submit` / `run_precommit` and the verdict recorder with the reason; `set_gate_mode loop|orchestrator` is clamped to `normal`. The output-language directive (L4) stays — it is orthogonal to the gate.
...
Handing over is **not** one of the orchestration tools: `session_handoff()` belongs to every kind of session (a plain loop session, an orchestration child, the project manager, a judge pane), and the gate owns the whole chain after the call — the document skeleton, the successor pane's first message, and closing the predecessor once the successor has demonstrably taken over. |


**The plan is the exit contract**, and it is the orchestration-layer twin of the loop goal: writing `.pi/orchestrator-plan.json` grants nothing, because the approval binds to the **content hash** the user saw in a dialog the extension rendered. Widen a boundary, add a task, or move a task to another repo afterwards and the approval is gone. Every task must declare its `repo` (2026-09-01) — that one field is what makes scheduling and proxy-approval decidable at all; **which files it touches is no longer part of the plan** (2026-09-17: at that time same-repo tasks WERE serialized by the gate, so a file boundary prevented nothing and only forced a re-audit every time a directory was added). Same-repo parallelism is back (2026-09-10): `orchestrator_spawn` gives the second writer its own `git worktree`, and the manager settles that checkout with `orchestrator_close({worktree: keep|merge|discard})` — where `merge` now also **reclaims the directory** once the merge succeeds (2026-09-15), keeping only the branch as the `merge --abort` anchor. **One requirement, one PR per repo** (2026-09-15, user decision): when a repo holds more than one task, its delivery station is narrowed to `commit` unless the user's own plan names that repo in `allowMultiplePrs` — each child stops at a local commit, and the plan's TAIL is TWO tasks (**positions in the list, never a new field**, 2026-09-22): the **second-to-last** is the wrap-up task — it settles the siblings' branches, takes the whole through one review and commits (the narrowing caps it like any other task); the **last** is the independent acceptance task — no new requirement, no business code, it runs the real acceptance on the changed system and then delivers (push + the single PR). Only the acceptance task is exempt from the narrowing; without a delivery task at the end an orchestration has nobody left who may publish — the manager is forbidden to write code, and every child was capped. Adding a repo to `allowMultiplePrs` is a widening (the approval is revoked and the user is asked again); removing one is an amendment. A goal the orchestrator approves for a child may not name a path outside its task (that is a scope change, and scope belongs to the human).

**Addressing is by orchestration, not by session.** A child's wake-ups are stamped with `RG_ORCHESTRATION_ID`, not with the id of the session that spawned it — so when a relay hands the role to a successor, every running child keeps reaching whoever holds it now, with nothing restarted and nothing re-stamped. The relay's closing move is the guarantee that there is no gap: **only the successor may close the predecessor**, which means the old session stays alive until something demonstrably running has taken over. It inherits the id, the handoff document, a pointer to the predecessor's **transcript** (a handoff document is a self-report; the raw record is what you need when the plan later goes wrong), and the predecessor's **session id** — its proof of heirship. Retiring the predecessor is TWO PHASES, not a flag (2026-09-10, after a measured failure and the review round it took to get the split right): phase one RELEASES the worktree claim before the successor's pane opens (the successor arms its gate in the SAME checkout — releasing after the pane opens is racing its boot, and it lost: the successor was refused by its own predecessor's heartbeat and its pi exited, leaving the pane on a bare shell); phase two goes SILENT — the retirement flag, both wake-up timers, and the sidecar writer — only once the successor's relay record is persisted, because `persist()` refuses to write for a retired session and silencing earlier would make that record memory-only. Every wake-up path honours the flag (`orchestratorSettled` had none, so `agent_settled` put the predecessor back into `orchestrator_wait` two seconds after a successful handoff — two project managers, one plan). A relay that never starts its successor rolls phase one back and never enters phase two; the successor's heirship claim (it carries the predecessor's session id) means even an un-released claim cannot block the handoff.
...
L9  Real acceptance       loop mode → the GATE itself dispatches the
                          ...
                          finds code, an approved acceptance plan on record
                          (the goal's 真实验收方案 — no plan, no dispatch),
                          and no verdict bound to the current
...
| **`acceptance`** (`agents/acceptance.md`) | *at task COMPLETION* — the GATE dispatches it itself the first time `declare_done` finds code, an approved acceptance plan on record and no verdict bound to the current tree: it runs the goal's 「真实验收方案」 on the REAL system (start it, call it, compare the answer, re-check the neighbours) instead of reading the diff | yes (READY/BLOCKED) — a non-READY, or a READY bound to content that has since moved, refuses `declare_done` | Fable 5 → Opus 5 | `max` |
...
persist, so including it would let a recorded verdict immediately invalidate its
...
**Judge roles (reviewer / adviser / goal-auditor / quality-auditor / acceptance) run in their OWN tmux panes** — interactive pi with a deterministic session id per role+repo, `--exclude-tools edit,write`, resumed by that id when the pane is gone. No workflow engine, no background subagent.
...
commit range, no second reviewer. (Read-only WORK needs are a different family —
`worker_submit` / `worker_wait` / `worker_answer` / `worker_close`, 2026-09-21 —
and a worker never dispatches a judge.)
...
judge_conclude({
  ...
  findings: [{file: "src/x.ts", line: 42, severity: "P1", issue: "…", evidence: "<optional>"}],
})
```

Those fields are written straight into the round's channel `report` record and
the opener consumes them as data — no fence is synthesised and none is parsed.
The signature is role-shaped: a `reviewer` and a `goal-auditor` have **no**
`notes` parameter (passing one is refused, and the refusal does not spend the
round's single conclusion), because their conclusion IS the verdict plus the
findings. An `adviser` keeps `notes` — its product is the prose, and the opener
quotes it back.

Precommit verdicts are a different gate and are NOT parsed from
...
- **Then the goal-auditor — mechanically (since 2026-08-25).** It is ONE call:
  ...
  The **extension** reads the auditor's own structured conclusion off its
  channel report (PASS ⇔ a `READY` verdict with no unresolved P0/P1 finding)
  and hashes the audited text itself — there is no `passed` parameter, so the
  agent cannot ATTEST the outcome, and there is no recorder tool it could call
  with a hand-written verdict either: the recorder is a plain function the gate
  invokes when the round lands. A FAIL means: fix the
...
- **Edits are gated too (since 2026-08-23).** In loop mode (and while the mode
  ...
  audit itself, then shows the dialog). The approval dialog is the gate's ONE
  template: `认可，写入 .pi/loop-goal.md（推荐）` / `不认可，退回重谈` /
  `✎ 我要改，我说明原因` — the rejection's reason is typed into that same box
  (2026-09-08) and carried back to the agent for renegotiation; there is no
  second dialog.
  (One deliberate exception: a judge child's own THROWAWAY worktree is inert —
  the L8 edit gate does not block a reviewer's verification run inside the
...
- **The wait is EVIDENCE, not a poll.** Measured on real PRs (50 paired rounds,
  ...
  - **queued / working** — the request is pending, and `copilot_work_started`
    from the timeline says whether Copilot has begun. The call then **blocks**
    on it (a light ~1 KB query every 20–45 seconds, `lib/copilot-watch.ts`
    `awaitCopilotNews`) and reads the review in the same call the moment it
    lands; ESC or typing interrupts, calling again resumes. No turn is ended to
    wait (2026-09-23: a background wake used to stand in for this, and an
    orchestration child that ended its turn read as idle, waking its manager
    every minute). The wait used to be the agent's own — 3 × 20-second polls
...
- **From round 4 on, the USER triages the findings.** The first rounds are
  ...
  `✎ 不选，我说明原因` row. The reply comes
  back grouped by that decision, and **only the `修复` group is the agent's to
  change**: a finding nobody answered is *not* approval — the code is left
  alone and the finding is reported and asked again. Rules live in
  `lib/copilot-triage.ts` (threshold, the thread+last-comment key that makes a
  new Copilot comment a new question, and the grouping); the dialogs are
  rendered by the extension, so a project manager can answer them for a child
  session exactly like every other gate question.
...
- **Explicit target repo**: the judge tools (`judge_submit` / `judge_spawn` /
  `judge_answer` / `judge_recover`) take a `repo` argument, and it is **mandatory
  once the session has edited more than one repo** — they refuse to guess, and
  so do the steps a round runs (the verdict recorder / `run_precommit`).
...
| `/gate-contract` | Show the contract THIS session is working to: a project manager gets its plan tasks (four states, with the ids each blocked task waits on), a loop session or an orchestrated child gets the exit criteria of its own user-approved goal, verbatim and in full. On demand rather than permanent (2026-09-18, user decision) — nothing here is worth stealing editor rows between asks, so there is no truncation and no fold. When the session owns no such contract it says why (goal unapproved, no plan yet, judge pane, non-git directory) instead of printing an empty block. Read-only: no state changes, no git, no shortcut registered — bind one in `/settings` if you want one |
...
> `prepare_review`, `prepare_adviser` and `prepare_goal_audit` are **still the
> implementations** the gate runs —
...
> see the names. `record_review` and `record_goal_prereview` went further on
> 2026-09-04: they are not registered anywhere at all, on any host, and are
> plain functions the gate calls when a round's report lands.
> What all of them do is still exactly how the gate works, so their
...
| `ask_user` | The ONE way to reach the user — requirement ambiguity, a product/design decision, scope trade-offs, the loop-goal interview. Calling it **pauses** the loop until the answers come back, which is why a question written into the reply and an ended turn is not an alternative: that costs a whole iteration and may not even read as a question. EVERY question follows the gate's ONE dialog template (user decision, 2026-09-08): 2–4 options, each drawn with its own letter (`A. text`, the recommended one marked （推荐）), plus the template's own row `✎ 不选，我说明原因` which opens a MULTI-LINE editor (pi's own component: newlines, paste, `ctrl+g` into your `$EDITOR`) — so the user can always answer with a reason instead of picking anything. A question with fewer than 2 options or no recommendation REJECTS THE WHOLE BATCH with no dialog shown; more than 4 options is truncated and reported. There is **no cap on how many questions one call may ask** (user decision, 2026-09-17: the old 10-question cap dropped the tail of a long batch, and an agent told the rest would come "next round" usually never asked again). The extension runs the interview itself (one question at a time with its `N / M` progress; **closing a box stops the rest**, which come back unanswered; `!chat` is the one typed escape the reason editor honours), and every answer returns at once with the unanswered ones marked — written as the screen wrote them (`→ A. text`). TWO WAYS BACK, both the template's own (user decision, 2026-09-19): every question after the first offers `← 返回上一题`, which re-asks an earlier one and overwrites only THAT answer (a re-answered authorization question takes its proxy scope back); and ESC in the reason editor returns to that question's list with what was typed kept, while the LIST's ESC is what still closes the question (and, in an interview, stops the rest). It replaced `pause_for_question`, which was deleted on 2026-08-29: that tool only *paused* and carried exactly one question, leaving the agent to restate it in the reply — two ways to reach the user, one of which delivered nothing. Asking permission to continue routine loop work is still prohibited. |
| `judge_submit` | The ONE entry point for a judge round (`reviewer` / `adviser` / `goal-auditor`). The call passes WHO and WHAT; the gate owns everything procedural — session id, working directory, pane open vs. channel-queued vs. fresh kill, the channel verdict — so no session id, title or directory is ever passed in. For `reviewer` it runs the whole chain itself: the FULL precommit (started to run beside the chain, so its FAIL arrives as a separate steered message naming its round and the content it verified), the checkpoint commit, the `baseline..HEAD` computation and the finding-stream file, then the dispatch; any other step that fails sends the round back with the reason instead of leaving it half-submitted. Each review runs in its OWN tmux pane (interactive pi, deterministic session id per role+repo, review-gate loaded in judge mode, `--exclude-tools edit,write`); the `subagent` dispatch surface was retired 2026-09-06 with the pi-subagents companion, so `judge_submit` is the only way a checkpoint round can run. It returns as soon as the round is SUBMITTED, not when the judge is done: the round ends when its channel report lands, and the opener records the verdict from the report's exact bytes. A living pane takes every new round through its channel (nothing is silently dropped); `fresh: true` kills the pane first. POLISH GATE: after two consecutive READYs, or the same file polished for three rounds, a reviewer round without a `reason` is refused, and the reason travels into the reviewer's task text. |
| `judge_wait` | Wait for YOUR OWN judge's next MESSAGE and return it — MESSAGE-DRIVEN, not a poll-until-finished: a newly streamed finding, a question the judge asked, the round's channel report (the gate records it) or a dead pane each return immediately, carrying the content itself (finding bodies, the question with its options, the verdict and record note) so nothing has to be read out of a stream or transcript file. Per-judge cursors make each message arrive once: the consumed report id and finding count live on the judge's registry entry, and the announced question ids are shared with the settle path. Only the opener may wait. It is the FALLBACK, not the reflex — do the deterministic work you have first (`lib/agent-directives.ts`'s `buildWaitDiscipline` is the one wording of that rule, and the gate's settle wake-up remains the backstop when you are not waiting). It came BACK to the agent surface on 2026-09-05: with the gate forbidding "end the turn to be woken" and no waiting tool available, the only move left was a hand-written `sleep` loop, which never settles — a measured nine minutes with a finished review sitting unrecorded. |
...
THE PANE LIFECYCLE, 2026-09-21 (user decision): **one policy — a judge pane is freed when its round is recorded**, whoever dispatched it. It replaced the 2026-09-06 split (the gate's own auditor died at round end, an agent-dispatched review pane lived until `declare_done`), because a recorded verdict IS the deliverable and a pane is screen space. Freeing one costs no context: the next `judge_submit` for that role finds a dead pane and re-opens the SAME session id, so the transcript continues. `declare_done`'s cascade is still the terminus for a pane whose round never concluded. `judge_close` is not on the agent surface (it exists only for the gate's own chains).

WORKERS (2026-09-21). `worker_submit({task, workerId?, role?, model?})` dispatches a READ-ONLY pane session — the pane-shaped replacement for `npm:@tintinweb/pi-subagents` (`Agent` / `SubagentWorkflow`), and the answer to "opening a subagent and opening a review pane should be the same thing". Same pane factory, same channel, same deterministic session id; `--exclude-tools edit,write` keeps the WRITING tools off its surface, which is what makes several workers safe to run at once. `bash` is deliberately NOT excluded (user decision, 2026-09-22): a worker that cannot run `git log`, `rg` or a test investigates nothing — so "read-only" for commands is a PROMPT rule (the worker's system prompt forbids writing files and names the read-only uses) backed by the gate's own ship block, not a surface fact. A LIVING pane receives a second submit as a message (not a second worker); a CLOSED one is re-opened under the same session id, so `worker_close` frees screen space without losing the conversation. `worker_wait` returns the worker's report or the question it is blocked on (it waits through `lib/poll-wait.ts`'s `pollUntil` like every other wait in the gate, so ESC — or the user simply typing — ends it immediately and consumes nothing: the report cursor only moves on a report actually handed over), `worker_answer` answers it. What each preset runs is configured in `~/.pi/review-gate.json` under `agents.worker*` (a model chain plus its own `prompt`); an unconfigured preset FAILS the dispatch instead of running on a default model.
...
| `orchestrator_plan` / `orchestrator_spawn` / `orchestrator_wait` / `orchestrator_answer` / `orchestrator_instruct` / `orchestrator_notify` / `orchestrator_recover` / `orchestrator_attach` / `orchestrator_close` | The orchestration layer, available only in `orchestrator` mode — see [The orchestrator role](#the-orchestrator-role-a-project-manager-inside-the-gate). The decisions live in `lib/orchestrator-*.ts` (plan state machine, the plan pre-audit, whether an edit widened anything, file-boundary algebra, the supervision channel and its child states — `CHILD_STATES` in `lib/orchestrator-child-state.ts` is the list, and nothing else keeps a copy of it — pane decoration, tmux argv construction, the bash backstop, the 14 constraints); the extension only wires them up. Handing over is a separate family of one: `session_handoff()` for every session kind (`lib/session-handoff.ts` + `lib/session-handoff-tools.ts`). |
| `declare_done` | Completion claim, **re-validated server-side** — rejects with `isError` if any gate is unmet (the reject hint reminds you that late doc/handoff edits invalidate the READY fingerprint, so finish all edits before the final review). "Declaring ≠ executing." It also enforces the COMPLETION-only requirements the ship gate deliberately does not carry: an open Copilot review cycle (L7), an unapproved loop goal (L8), — in loop mode — ARRIVAL at the round's delivery station (`commit` needs a committed worktree; `pr` additionally needs evidence that a PR was opened — a `gh pr create` the gate itself watched succeed, or a PR number the Copilot cycle resolved; `precommit` adds nothing) — and, since 2026-09-22, the REAL-ACCEPTANCE round (L9): when the round carries code, an approved acceptance plan is on record and no verdict is bound to the current content, the gate dispatches the `acceptance` judge ITSELF and tells you to `judge_wait({role:"acceptance"})`; a non-READY verdict refuses completion with its findings, and the requirement stays out of `unmetRequirements` on purpose (fixing a finding needs a commit, so a ship-level requirement would block its own remedy). On accept the work **stays on the branch it was done on** (2026-09-07: the gate no longer merges anything — merging/rebasing/pushing is the user's own git workflow). It also clears the per-task round history so a subsequent task in the same session starts its round counter fresh. |
...
These three are **not tools**: they are registered into the extension's own
...
submission chain. That is how every
...
The two RECORDERS went one step further (2026-09-04): `record_review` and
`record_goal_prereview` are gone from every surface, internal included, and are
plain functions the gate calls when a round's report lands
(`recordReviewVerdict` in the extension, `recordGoalPrereview` in
`lib/goal-prereview-tools.ts`). Their tool shape existed only to carry raw text
that had to be parsed back into a verdict; the conclusion arrives structured
now, so there is nothing left for a caller to pass. What they DO is unchanged
and described under [the review loop](#the-review-loop) and the loop-goal
section above: the commit-target integrity check (a READY is withheld when the
round was never prepared, downgraded to BLOCKED as STALE when HEAD moved past
the reviewed commit, and bound to the reviewed commit's TREE), the `cwd`
consistency check, the round history for plateau/oscillation detection, and the
L8b PASS record bound to the audited draft's hash.

| Internal step | What it does |
|---------------|--------------|
| `prepare_review` | ADVANCED / internal — `judge_submit({role:"reviewer"})` runs this itself as step 3 of the chain. Registers the COMMIT target for the single reviewer of this round: requires the checkpoint from `review_checkpoint` (the only commit allowed before a READY), computes the immutable range `baseline..HEAD`, writes the append-only finding-stream file and returns the ready-made task text. In a copy of its own the reviewer reads the range first and runs nothing by default; a concrete doubt buys the minimal verification (`docs/judge-protocol.md` 「验证纪律」) — while the main agent keeps fixing the real worktree and consumes the stream as it lands. A READY recorded after HEAD moved (a new checkpoint during the review) does NOT bind: the verdict recorder compares HEAD with the registered reviewed commit and downgrades to BLOCKED (STALE), so an approval can never cover commits no reviewer saw. A READY must also carry the judge's own `pwd`, which the recorder compares with the reviewed repo. Round-18 POLISH GATE: when the last two recorded rounds both verdict READY, or the same file has carried P2/Nit findings in three consecutive rounds, the tool REFUSES a `reason`-less call; the supplied reason is persisted (`lastPolishReason`) and injected into the next reviewer's task text, so a "polish" round is visible to the independent judge. |
...
- Forging a judge's channel `report` record on disk — the reviewer and the
  goal-auditor are separate pi processes and their conclusions reach the gate
  through the channel file, which the main agent's own process can write (it
  can equally write the sidecar directly), so both verdicts rest on the
  cooperative assumption. What the extension DOES guarantee is that no
  agent-attested boolean is accepted, and that there is no TOOL to attest one
  through: `judge_conclude` is registered only inside a judge session, and the
  two recorders are plain functions the gate calls when a round's report lands
  — the agent has no call it could make with a hand-written verdict. Making it
  unforgeable would require the extension to verify a receipt from the judge
  process, like `run_precommit` (future hardening), not a runtime source check.
...
enforcement path — ship blocks, `declare_done`, the verdict recorder, arbitration,
...
lib/copilot-triage.ts         L7's user half (pure): the round threshold that starts asking, the
                              thread+last-comment key, the four decision groups, triage sanitize
...
agents/acceptance.md          real-environment acceptance judge, dispatched by the gate at completion, pinned model @ max
...
lib/thinking-loop-guard.ts    pure-thinking spinning detection (upstream deepseek-harness#5976): reasoning-only turn + a long UNBROKEN low-entropy repeat → verdict, plus the char-AND-line bounded display cut (pure)
lib/thinking-loop-controller.ts  what to do on a trip — inject, abort, notify, with a capped auto-recovery counter; the display cut is CONTENT-addressed (a markdown transformer never sees a message id); every effect is injected, so an event sequence is a unit test
...
lib/precommit-parse.ts        precommit `## Overall:` sentinel parser (worst wins; FAIL terminal)
lib/review-adjudicate.ts      reviewer adjudication over the judge's STRUCTURED conclusion: READY+open P0/P1 → BLOCKED, findings count, coarse cross-round fingerprints
