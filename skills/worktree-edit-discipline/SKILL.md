...
上一轮复盘记录的是：`insert` 会在**字符串拼接 / 参数列表中间**塞进空行；直接删那个空行是 **noop**，必须连相邻锚点一起替换。

**本轮四次实测未能复现这两条。** 探针留在 `/tmp/pm-rounds/evidence/t3b/insert-probe.ts`，覆盖上一轮点名的两种形状：
...
**所以：不要把「`insert` 必然塞空行」当成既定事实去做预防性重构。** 上一轮的观测是真实的，但它要么已被修复、要么依赖某个本轮未触及的特定形状——在能复现之前，它不是一条可依赖的规律。
