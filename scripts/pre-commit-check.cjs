 ...
 * process.exit calls are intercepted for the in-process call (HookExit /
 * interceptor below), and the divergence fingerprint's stdout is captured the
 * same way — the hook's own stdout stays empty, exactly as it was when bash
 * captured FP_JSON.
 *
 * Exit codes mirror the old hook chain: 0 = allow, 1 = block (any reason).
 * The REVIEW_GATE_BYPASS=1 fast path never reaches this file (the shell
 * handles it before exec).
 *
 * Usage: node pre-commit-check.cjs <stateFile> [--repo <root>]
...
// into a bare MODULE_NOT_FOUND crash. Each site below requires what it needs.
...
/** process.exit interceptor — the in-process checkers must not kill the hook. */
...
/** Run a checker body that exits via process.exit; returns its exit code. */
...
//    shell hook allows; here they map to exit 0.
// ---------------------------------------------------------------------------
...
// Project config wins; the user-global config is the fallback.
// ---------------------------------------------------------------------------
function readDocSyncFlag(path) {
  try {
...
if (labels) {
  ...
  if (code === 1) process.exit(1); // violations already printed by the scanner
} else {
...
//    separate compute fallback). The checker's stdout fingerprint is captured
//    here — the hook's own stdout stays empty, as before.
//    Missing script → FAIL CLOSED (the one guard for a core safety property).
// ---------------------------------------------------------------------------
...
let fpJson = "";
{
  const argv = [process.execPath, divergenceScript, repoRoot, process.env.GIT_INDEX_FILE || "", "--emit-fingerprint"];
  const captured = [];
  ...
  if (code === 1) process.exit(1); // divergence printed its block reason
  fpJson = captured.join("\n");
}
...
// separately. Correctness is unaffected — only single-materialization is lost.
if (!fpJson.trim()) {
...
// ---------------------------------------------------------------------------
// 4. Verdict comparison (was the second inline node heredoc). A worktree with
//    nothing tracked is not an error state — it exits BEFORE the verdict
//    chain, exactly where the old hook did (AFTER the L6/divergence checks,
//    which run unconditionally past bypass).
// ---------------------------------------------------------------------------
...
// FINGERPRINT ALGORITHM MIGRATION — recognise an old-algorithm binding and say
// what to do, while still failing closed.
...
if (state.hasCodeChange) {
  ...
  // Round-9 P1 / round-10 P1: the unreviewed-commit check is a STANDALONE if,
  // NOT an else-if on the fingerprint chain; it runs only when the review
  // passed every check above (problems.length === 0 — a length guard, not
