...
| `lib/session-factory.ts`（新建，2026-09-05） | **开一个带角色的 pi 会话的唯一入口**：split → 登记 → 装饰 → 等分 → 投递核实一次做完，judge 与编排子会话（含两处 recover 与接力 successor）共用。收敛前每处各写一遍，代价是三个实测缺陷：judge pane 的边框行没人开（C1）、judge 标题没人刷新（C2）、judge 侧没有投递核实。argv 仍只在 `orchestrator-tmux.ts` 构造，颜色/标题字符串仍在 `orchestrator-pane-decor.ts`；落点与等分（2026-09-08 的三列规则）也是它调那两处的，agent 不碰窗口 |
