...
| `lib/session-name-tools.ts` + `lib/session-registry.ts` + `lib/session-orphan-sweep.ts`（新建，2026-09-25，t2） | **会话自己的名字**：`name_session({name})` 占一个全局唯一的名字（`~/.pi/agent/rg-sessions/<名字>.json`），写 window title 与 window 级 `@rg_session_name`（状态栏读它，显示「目录名 · 会话名」），心跳续期，`declare_done`/进程退出释放；启动时回收死会话留下的 tmux session/登记/inbox。名字形状、原子占用、live/dead/unknown 分类、fail-closed 全在 `session-registry.ts`，回收（含标记比对与 kill）单独立在 `session-orphan-sweep.ts`（它杀别人留下的 session，而注册表只读一个名字），扩展只接线四个时刻。同一 repo 别的会话的名字也能被读到 —— 它是 t3 「@名字 发消息」的地址来源 |
