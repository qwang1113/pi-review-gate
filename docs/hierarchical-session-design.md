...
| `lib/session-message-tools.ts`（新建，2026-09-25，t3） | **按 @名字 直接发消息**：`send_message({to, text})` 查 t2 的注册表拿到活会话 ⇒ 写对方 inbox（`~/.pi/agent/rg-sessions/<名字>.inbox.jsonl`）⇒ 对方门禁在自己的轮询里读到后用 `pi.sendUserMessage(text, {deliverAs:"steer"})` 注入成一条标明「来自 @谁」的 user 消息，**不中止对方当前的 turn**。发送侧判定（`@` 前缀可省、未命名不能发、非法/不存在/dead/unknown/发给自己都拒绝并列出当前活会话）、写盘（复用 `orchestrator-channel.ts` 的单次 `O_APPEND` + 超长 spill 约定）、收件侧消费（取件 rename ⇒ 注入 ⇒ 成功即删 / 失败留原地下次重试）全在这里；路径一条来源（`sessionInboxPath` + 派生的 `.taken`）。所有会话（loop / PM / 子会话 / judge / worker）都注册它、都收消息，扩展只接线两处：`register(pi)` 与把 `drain()` 挂在 t2 已有的命名心跳定时器上（不新增第二个心跳）。**结构化通道（PM 指派 / 子会话回报 / judge 结论 / ask_user 代答）一行不动** |
