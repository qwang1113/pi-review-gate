...
* **复用的三条边界（2026-09-05 落地，`lib/judge-rotation.ts`）**：复用不能没有释放点，否则一条 transcript 会把整个项目的审查越积越多地攒在一起（实测样本：2.8MB、累计 $114、上下文 74.8%）。所以门禁给复用同时定义了单元、释放点与上限：
  ...
  * **上限 = 70% 上下文 / 同对象 8 轮**，任一命中门禁自己轮转（代次 +1）。70% 是**统一交接阈值**（`HANDOFF_PERCENT`，本机 1M 窗口 ⇒ 700k）：judge 自己 60% 那个数字已删（2026-09-14，一个决定只有一个阈值）。上下文读数由 judge 侧在交卷时随 report 上报（opener 读不到自己以外进程的用量）；**读不到就不因它轮转**（fail-open，只靠轮次兜底，否则每一轮都会变成新 transcript，等于取消复用）。轮次在**派发时**计数，所以被放弃/`fresh` 重开的轮也算——否则反复重开就能绕开上限。
...
| `lib/session-factory.ts`（新建，2026-09-05；2026-09-25 改 window 模型） | **开一个带角色的 pi 会话的唯一入口**：开 → 登记 → 装饰 → 投递核实一次做完，judge 与编排子会话（含两处 recover 与接力 successor）共用。收敛前每处各写一遍，代价是三个实测缺陷：judge pane 的边框行没人开（C1）、judge 标题没人刷新（C2）、judge 侧没有投递核实。**落点是一个 window**（2026-09-25 用户决定）：子会话是 opener 专属 tmux session 里的一个 window，session 的命名/懒建/归属/kill 在 `lib/session-tmux-scope.ts`，三列布局与等分已整块删除；唯一仍 split 用户窗口的是接力后继者（同一位用户决定）。argv 仍只在 `orchestrator-tmux.ts` 构造，颜色/标题字符串仍在 `orchestrator-pane-decor.ts`，agent 不碰窗口 |
| `lib/session-name-tools.ts` + `lib/session-registry.ts` + `lib/session-orphan-sweep.ts`（新建，2026-09-25，t2） | **会话自己的名字**：`name_session({name})` 占一个全局唯一的名字（`~/.pi/agent/rg-sessions/<名字>.json`），写 window title 与 window 级 `@rg_session_name`（状态栏读它，显示「目录名 · 会话名」），心跳续期，`declare_done` / 会话被替换（非 `reload`）/ 进程 `exit` 时释放；启动时回收死会话留下的 tmux session/登记/inbox。名字形状、原子占用、live/dead/unknown 分类、fail-closed 全在 `session-registry.ts`，回收（含标记比对与 kill）单独立在 `session-orphan-sweep.ts`（它杀别人留下的 session，而注册表只读一个名字），扩展只接线四个时刻。同一 repo 别的会话的名字也能被读到 —— 它是 t3 「@名字 发消息」的地址来源 |
| `lib/session-message-tools.ts`（新建，2026-09-25，t3） | **按 @名字 直接发消息**：`send_message({to, text})` 查 t2 的注册表拿到活会话 ⇒ 写对方 inbox（`~/.pi/agent/rg-sessions/<名字>.inbox.jsonl`）⇒ 对方门禁在自己的轮询里读到后用 `pi.sendUserMessage(text, {deliverAs:"steer"})` 注入成一条标明「来自 @谁」的 user 消息，**不中止对方当前的 turn**。发送侧判定（`@` 前缀可省、未命名不能发、非法/不存在/dead/unknown/发给自己都拒绝并列出当前活会话）、写盘（复用 `orchestrator-channel.ts` 的单次 `O_APPEND` + 超长 spill 约定）、收件侧消费（取件 rename ⇒ 注入 ⇒ 成功即删 / 失败留原地下次重试；每条消息记着**发给哪个 session**（`toSessionId`）—— 名字换主人后，接管者跳过不属于自己的邮件，而不是被播到别人的往来）全在这里；路径一条来源（`sessionInboxPath` + 派生的 `.taken`）。**任何一方都不删别人的邮件**（release 与 sweep 都不清 inbox 与 spill 文件）：撤下名字是原子的，新会话随时可以抢到它并收到信，那时清理就是跨会话丢信 —— reviewer P1 两轮都打在这里，所以删除动作用「不删 + 收件人绑定 sessionId」根除，而不是用 CAS 缩窗。所有会话（loop / PM / 子会话 / judge / worker）都注册它、都收消息，扩展只接线两处：`register(pi)` 与把 `drain()` 挂在 t2 已有的命名心跳定时器上（不新增第二个心跳）。**结构化通道（PM 指派 / 子会话回报 / judge 结论 / ask_user 代答）一行不动** |
