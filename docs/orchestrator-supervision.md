...
**但完成记录必须是关于「当前这次派活」的**（第一轮 review 的 P1）：`declare_done` 写下的记录会一直躺在 sidecar 里，
如果不比时间，一个被派了第二件事的子会话只要屏幕一静止就又被判 `done` —— 而那和它**卡在新任务上**长得一模一样，
于是唯一不响铃的状态会吞掉唯一必须被听见的情况。

完成记录因此有**两道互补的失效条件**，一道在编排层、一道在子会话自己身上：

1. **编排层按时间**：`orchestrator_spawn` 与每一次送达的 `orchestrator_send` 都会在登记表上盖一个 `lastAssignedAt`，
...
2. **子会话按事实**：它**每编辑一个项目文件**（源码或文档，`.pi/` 与非项目文件不算），门禁就顺手清掉 `completion` ——
   就在它把 READY 降回 PENDING 的同一处。这条覆盖编排层看不见的那条路：**人直接在 pane 里给它派新活**。
   一个又开始编辑的会话就是在干活，不管是谁让它干的。
...
1. **结构化真值（最可信）** —— `ChildSidecarFacts`，由 `lib/orchestrator-probe.ts` 的 `structuredFacts()` 组装：
   ...
   - `pausedQuestion` / `reviewVerdict` / `precommitVerdict` / **`completedAt`**：从子会话自己的门禁 sidecar
     ...
     它**只描述写下它的那一轮工作**：子会话之后任何一次项目文件编辑都会清掉它（§1.1 的第 2 道失效条件），
     编排层再按 `lastAssignedAt` 比一次时间。
