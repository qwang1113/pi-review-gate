...
**文件边界于 2026-09-17 被用户从 plan 中删除**，这一整类编辑因此不再存在：自
2026-09-07 起同一 repo 的任务本来就被串行调度（`scheduleNextTasks` 只按 `repo` 判
冲突），边界防不住任何冲突，只剩「每次新开一个目录都要重新审计 + 重批」这一个作用。
...
是否真的分在不同 repo（同一 repo 会被降级为串行）、依赖是否成环或缺失、
