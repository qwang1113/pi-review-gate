...
区分顶层文件与顶层目录，猜错就等于把一个文件放大成整个仓库，因此它只覆盖自己）；
...
目录型边界（`test`）与顶层文件（`README.md`）则只覆盖自己。它是
`approvedTree()` 一个函数说了算，回执文案与判定共用它，所以拒绝时能直接把该任务
的目录树列给项目经理看。
...
两道锁防止「铸造」这条豁免：状态只能来自执行记录（`write` 改不了状态，
`mergeTaskProgress` 从上一份 plan 抄回，只有 `set-status` 能改），且该任务必须
存在于**已批准快照**里 —— 新任务自带 `status: "done"` 释放不了任何东西。同一路径
若还被一个**仍在进行**的任务声明，照样拒绝，并指名那个任务。
