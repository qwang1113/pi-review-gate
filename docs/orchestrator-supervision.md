...
**什么时候到取决于它是怎么发的**：`steer` 切进当前这一轮，`followUp` 要等这一轮
turn **结束**才送达。所以回执明说那唯一总是有效的动作：**别立刻回到 wait 里去**，
先把手上这一轮收掉。否则项目经理会掉进同一个坑的第二跳 —— 被消息叫醒、转身又进
900s 阻塞，而那条 followUp 仍在队列里等 turn 边界，等于把原缺陷重演一次。
