...
> 历史溯源：本文出现的 R-xx / F-xx 编号来自 `docs/e2e-findings-2026-08-29.md` 的端到端复盘。
...
3. `event.fromPane` 缺失（老版本发布者没盖来源）→ **采信**，但在回执里标注无法核对归属 —— 拒收它可能让一个真实的求助失声。
...
- **不要自己跑 `tmux capture-pane` 轮询。** 项目经理会话里的 tmux 命令会被拦，而且探针已经在做同一件事，
  两套观测只会互相打架。
