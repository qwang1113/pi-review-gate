...
- **子会话的问题走点对点通道**（2026-08-30）：`propose_loop_goal` 的批准框、
  ...
  **物理的**（一个子会话一个文件），记录里根本没有收件人字段可填错。**这条路径
  不发任何桌面通知**：子会话的问题由项目经理答；真正需要你的是门禁自己的三类事件
  （`lib/user-notify.ts`：完成 / 异常结束 / 停下来等你回答 —— 第三类有两个入口，
  弹出对话框，以及登记一条待你拍板的 plan 决策），而项目经理自己弹的框就是其中之一。
...
- `review_checkpoint`：`git add -A && git commit`（英文 message 校验，
  提交信息是普通提交，门禁不再往里注入任何标记）→ 记录 commit sha。只绕过 READY，
...
- **precommit FAIL 不终止质量轮，但它的结论也不再是放行权**：lane 挂在一棵内容上时，质量轮继续跑
  完。它带来的价值是 **findings**——修完必然产生新 checkpoint（HEAD 移动），那份 READY 会被
