...
| `ship-detect.ts` | 判断一条命令行是否含 ship 操作（git commit/push、gh pr create/edit）；另有 `containsHeredoc`——**只给证据侧用**的收紧判定：同一份检测既用来拦（过匹配安全）又被交付站点用来放行（过匹配就是白给一张 PR 通行证），所以带 heredoc 的命令不作为到站证据，而**绝不**去教检测器认 heredoc（那才是真绕过） |
