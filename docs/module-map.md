...
| `orchestration-id.ts` | 编排 id：编排的稳定地址（不是 session id），接力换人后子会话无感。启动时持哪个 id 的判定也在这里（`startupOrchestrationId`，2026-09-17）：env 继承 → 本会话自己的 sidecar 运行时 → 否则新铸；**别的会话的 sidecar 一律不隐式接管**，那是 `orchestrator_attach` 的事 |
