...
| 交付站点定义句（precommit / commit / pr 各是什么）：权威 `lib/delivery-station.ts`（`describeDeliveryStation` / `describeDeliveryStationEn` / `DELIVERY_STATION_CHOICES_EN` / `deliveryStationChoiceLines`）；`lib/restatement.ts`、`lib/loop-goal.ts`、`lib/orchestrator-plan.ts` 一律**渲染**，`AGENTS.md`、`README.md`、`QUICKSTART.md`、`docs/dev-flow.md`、`skills/review-loop/SKILL.md` 只许摘要 + 指针 | `test/delivery-station.test.ts` · `"the station definitions live in ONE file — no other source restates them"` · `"every surface that summarises the station points at the module that defines it"` · `"the rendered choice lists are derived, not typed out again"` | 全仓（`lib/` + `extensions/` + `scripts/` + `hooks/` + 全部散文面）禁止第二份定义句字面量 + 摘要面必须回指模块 + 渲染器结构自证；第一条自带**针是真针**的自证（权威文件里必须能逐字找到六句定义） |
| orchestrator 工具清单：`AGENTS.md` 的「工具集（N 个）」、`README.md` 的工具表、`QUICKSTART.md` | 实际注册（`lib/orchestrator-*-tools.ts`，测试从 fake world 的注册表取真值） | `test/copy-convergence.test.ts` · `"every doc inventory of the orchestration tools is the set the gate registers"` · `"no doc presents a DELETED orchestration tool as one you can still call"` | 三处清单必须点齐每个注册工具（认 `_spawn` 简写）+ AGENTS.md 那个**数字**要等于注册数；第二条覆盖全部散文面：非注册的 `orchestrator_*` 只能出现在带「已删除/上一版/原来的…」标记的段落里（2026-09-17 实测：`docs/execution-model.md` 里三个死工具都在历史段落里，合规） |
