...
| `session-tmux-scope.ts` | **本会话自己的 tmux session**（2026-09-25 用户决定）：名字**永远由本会话身份派生**（`rg-<repo slug>-<开启者角色>-<session id 随机尾>`，角色 = `pm` / 编排任务 id / `self`，2026-09-27 s1 起让 `tmux ls` 可读；旧形状 `rg-<slug>-<尾>` 由 `sessionNameBelongsTo` / `scopeNameOwnedBy` 同样认领，角色中途变化时沿用 sidecar 已记录、仍属于本 repo+本 id 尾的那个名字；**取尾巴不取头** —— pi 的 id 是 UUIDv7，头是毫秒时间戳，同一分钟的会话前八位会一样）、**懒建**（`openScopeWindow`：首个子会话才建 session，且子会话自己的命令就是它的第一个 window，不留空壳 shell）、归属标记（session 级 user option `@rg_scope_owner`，复用与 kill 前都比对，对不上就既不接管也不杀），以及 `closeOwnSession`（`declare_done` 用它关掉自己建的那一个，幂等）。**sidecar 记录是事实而不是名字来源**（审查 P1）：只有记录与本会话派生结果**逐字段相符**（名字 = 我派生的、owner = 我的 session id）才被采信，否则视为“本会话没建过”的空操作 —— 手改或外来的记录因此既不能放宽执行器声明，也不能指使一次 kill。`list-sessions`/`list-panes -a` 读不到时一律「不知道」：不建也不杀。**可声明的 session 由标记证明**（2026-09-25 t4 整体审核 P1）：评判子会话的注册表行只当候选，每个候选要由 `createOwnershipProbe` 读 `@rg_scope_owner` 并确认「名字正是由该 owner 派生」才进声明 —— 此前只做形状检查，一份可写的注册表就能把声明放宽到别的会话的 session 上 |
...
| `tmux-pane-state.ts` | **本 pane 在做什么**（2026-09-27 s1）：每个 pi 会话（含非 git 目录的）用自己的 5s 定时器（`orchestrator-runtime-host.ts` 的 `startPaneState`，`session_start` 在非 git 短路之前启动，随命名心跳一起停）往本 pane 写 `@rg_sid` / `@rg_repo` / `@rg_kind` / `@rg_state`（`CHILD_STATES` 词表；对话框开着 ⇒ `waiting-input`，计数来自 `gate-dialogs.ts` 的 `dialogsOnScreen`）/ `@rg_state_at`；变化或满 30s 才写，一律 `set -p`，进程退出 `set -pu` 收回 |
| `tmux-sidebar-collect.ts` | tmux 侧边栏的**唯一一次读**：`list-panes -a` 的格式与解析（pane 选项 + session 的 `@rg_scope_owner` + window 的 `@rg_session_name` + 侧边栏自己的 `@rg_sidebar` 标记） |
| `tmux-sidebar-tree.ts` | 侧边栏的树：repo 分组 → pi 会话 → 它派出的子 window（靠子 window 所在 session 的 `@rg_scope_owner` 找开启者，可嵌套）、开启者已不在的孤儿、「其他」栏、`stalled`（`@rg_state_at` 超 90s）与「等你回答」计数。纯函数 |
| `tmux-sidebar-render.ts` | 侧边栏的屏幕：行文本（宽度按 CJK 双宽计）、行 → 跳转目标、选择/滚动/鼠标点击行号映射与着色。纯函数；终端循环在 `scripts/tmux-sidebar.ts` |
