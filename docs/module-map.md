...
| `dependency-justification.ts`（续） | 引用面测试钉住：`agents/reviewer.md`、`agents/goal-auditor.md`、goal/plan 审计任务、`MINIMALISM_REMINDER` 只许引用 §5 + 各自严重度映射（`test/agents-structure.test.ts` 等，见 §7.1 最小化行） |
| `dependency-justification.ts` | 新增依赖缺书面论证在 checkpoint 硬性打回的纯判定（`newDependencyNames` / `dependencyJustificationVerdict`），`review_checkpoint` 内接线；实现 §5“新依赖须论证”的机械一半，不复述其余三条 |
