...
| `agents-startup.ts` | 启动硬检查 `validateAgentsForStartup`（无内置默认；六个 judge 角色 + 已声明的 `agents.worker*` 预设）与缺口自愈 `healMissingAgentSlots`（只补 judge 角色），二者由 `startupAgentsCheck` 排序 |
