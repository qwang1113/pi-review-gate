...
| `tmux-pane-state.ts` | **本 pane 在做什么**（2026-09-27 s1）：每个 pi 会话用自己的 5s 定时器（`orchestrator-runtime-host.ts` 与命名心跳同起同停）往本 pane 写 `@rg_sid` / `@rg_repo` / `@rg_kind` / `@rg_state`（`CHILD_STATES` 词表；对话框开着 ⇒ `waiting-input`，计数来自 `gate-dialogs.ts` 的 `dialogsOnScreen`）/ `@rg_state_at`；变化或满 30s 才写，一律 `set -p`，进程退出 `set -pu` 收回 |
