...
agent：调 set_gate_mode("loop")
  → 有疑问就用 ask_user 问（一次一题，N of M；每题 2–4 个带 A/B/C 字母编号的选项 + 一个推荐，
     门禁自己追加「✎ 不选，我说明原因」那一行；第 2 题起还有「← 返回上一题」，理由框里按 ESC 退回列表；
     **不设数量上限**，没疑问也可以不问）
...
| 现象 | 原因 | 处理 |
|------|------|------|
| `edit/write 被拦 —— loop 模式下这个仓库还没有用户批准过的 loop goal` | loop 模式（含未决）还没确认目标就动手编辑 | 按三步谈出 goal：① `propose_restatement({restatement, station})` 把需求反述给用户确认（**硬前置** —— 没有已确认的反述，`propose_loop_goal` 直接被拒且不弹框）；② 用简体中文起草 goal（标识符/路径/代码 token 保持英文），格式照 `propose_loop_goal` 说明里的骨架填；③ `propose_loop_goal`（它自己跑 goal-auditor 预审，过了才弹对话框让你确认；被打回就按 findings 改完再提交一次）。批准前 edit/write 一律被拦（`.pi/` 与 `.pi-subagents/` 门禁自有文件除外；explore/normal 模式不要求 goal） |
| `propose_loop_goal 被拒 —— no goal-auditor pre-review has been recorded`（或 `…belongs to DIFFERENT text`） | 审计没过，或过了之后又改过字（hash 变了） | 按退回来的 findings 改草稿，**再调一次 `propose_loop_goal`** —— 它会重新跑审计（重审时自动带上一轮结论与草稿差异）。没有第二个调用要做 |
